...
class ProtocolBuffer
{
    ...
    public static function size_varint($i)
    {
        ...
        */
        ...
        if ($i < 0x8000000000000000)
            return 9;
        return 10;// this line is added by sinri to stop warning
    }
    ...
    public static function skip_field($fp, $wire_type)
    {
        switch ($wire_type) {
            case 0: // varint
                return ProtocolBuffer::skip_varint($fp);

            case 1: // 64bit
                if (fseek($fp, 8, SEEK_CUR) === -1)
                    throw new Exception('skip(' . ProtocolBuffer::get_wiretype(1) . '): Error seeking');
                return 8;

            case 2: // length delimited
                $varlen = 0;
                $len = ProtocolBuffer::read_varint($fp, $varlen);
                if (fseek($fp, $len, SEEK_CUR) === -1)
                    throw new Exception('skip(' . ProtocolBuffer::get_wiretype(2) . '): Error seeking');
                return $len - $varlen;
            ...
            case 5: // 32bit
                if (fseek($fp, 4, SEEK_CUR) === -1)
                    throw new Exception('skip(' . ProtocolBuffer::get_wiretype(5) . '): Error seeking');
                return 4;

            default:
                throw new Exception('skip(' . ProtocolBuffer::get_wiretype($wire_type) . '): Unsupported wire_type');
        }
    }
    ...
    public static function read_field($fp, $wire_type, &$limit = null)
    {
        switch ($wire_type) {
            case 0: // varint
                return ProtocolBuffer::read_varint($fp, $limit);
            ...
            default:
                throw new Exception('read_unknown(' . ProtocolBuffer::get_wiretype($wire_type) . '): Unsupported wire_type');
        }
    }
    ...
    {
        if (is_null($value))
            return "";// changed to return empty string by sinri
        ...
        if (is_array($value)) {
            ...
            if (is_object($value)) {
                self::$print_depth++;
                $ret .= get_class($value) . "(\n";
                $ret .= call_user_func_array([$value, "__toString"], []) . "\n";
                self::$print_depth--;
                $ret .= str_repeat(self::$indent_char, self::$print_depth) . ")\n";
