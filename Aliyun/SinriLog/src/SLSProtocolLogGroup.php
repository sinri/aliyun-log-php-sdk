...
class SLSProtocolLogGroup
{
    private $_unknown;

    /**
     * SLSProtocolLogGroup constructor.
     * @param null $in
     * @param int $limit
     * @throws Exception
     */
        ...
        while (!feof($fp) && $limit > 0) {
            ...
            switch ($field) {
                case 1:
                    ...
                    $limit -= $len;
                    $this->logs_[] = new SLSProtocolLog($fp, $len);
                    ASSERT('$len == 0');
                    break;
                case 2:
                    ASSERT('$wire == 2');
    ...
    function write($fp)
    {
        ...
        if (!is_null($this->category_)) {
            fwrite($fp, "\x12");
            ProtocolBuffer::write_varint($fp, strlen($this->category_));
            fwrite($fp, $this->category_);
        }
        if (!is_null($this->topic_)) {
            fwrite($fp, "\x1a");
            ProtocolBuffer::write_varint($fp, strlen($this->topic_));
            fwrite($fp, $this->topic_);
        }
        if (!is_null($this->source_)) {
            fwrite($fp, "\"");
            ProtocolBuffer::write_varint($fp, strlen($this->source_));
    ...
    public function __toString()
    {
        return ''
            ...
            . ProtocolBuffer::toString('logs_', $this->logs_)
            . ProtocolBuffer::toString('category_', $this->category_)
            . ProtocolBuffer::toString('topic_', $this->topic_)
            . ProtocolBuffer::toString('source_', $this->source_);
    }
